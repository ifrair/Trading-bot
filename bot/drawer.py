...
def draw_dataset(
    ...
    indicators: list = ["ADI", "CCI", "MACD", "MFI", "OBV", "PVT", "RSI"],
) -> None:
    """
    ...
    def to_range(
        ...
        :param r: max value
        """
        table[column] *= (right - left) / \
            (table[column].max() - table[column].min())
        table[column] += left - table[column].min()
